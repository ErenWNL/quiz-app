...
<c:set var="pageTitle" value="Edit Question" scope="request" />
...
<div class="container py-4">
    ...
    <div class="card border-0 shadow">
        ...
        <div class="card-body">
            <form:form action="${pageContext.request.contextPath}/admin/questions/${questionDto.id}" method="post" modelAttribute="questionDto">
                <form:hidden path="id" />
                ...
                <div id="options-container">
                    <c:forEach items="${questionDto.options}" var="option" varStatus="status">
                        <div class="row mb-3 option-row">
                            <div class="col-md-10">
                                <label class="form-label">Option ${status.index + 1}</label>
                                <input type="text" name="options[${status.index}].content" class="form-control"
                                       value="${option.content}" required />
                                <input type="hidden" name="options[${status.index}].id" value="${option.id}" />
                            </div>
                            <div class="col-md-2 d-flex align-items-end">
                                <div class="form-check">
                                    <input class="form-check-input correct-option" type="radio" name="correctOption"
                                           value="${status.index}" id="option${status.index}" ${option.correct ? 'checked' : ''} required />
                ...
                <div class="mb-3">
                    ...
                    <button type="button" id="removeOption" class="btn btn-outline-danger btn-sm ms-2"
                        ${questionDto.options.size() <= 2 ? 'disabled' : ''}>
...
<c:set var="pageSpecificScript">
    ...
    let optionCount = ${questionDto.options.size()}; // Start with existing options count
